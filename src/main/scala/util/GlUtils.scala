...
object GlUtils {
  ...
  def printBufferContents[BT <: Buffer, PT <: AnyVal](buf: PrimitiveBuffer[BT, PT]) {
    println(s"position: ${buf.position}, limit: ${buf.limit}")
    for(_ <- buf.position until buf.limit) {
      println(buf.get())
    }
  }
}