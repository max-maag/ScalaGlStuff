...
import model.{Sampler, GlFloat, Vector, GlUInt}
...
class JuliaScene(width: Int, height: Int) extends Scene {
  ...
  def start(): SceneStatus = {
    ...
    val sf = new ShaderFactory()
      ...
      .registerUniform("img", new Sampler(0))
      .registerUniform("maxIterations", GlUInt)
      .registerUniform("minVal", new Vector(2, GlFloat))
      .registerUniform("maxVal", new Vector(2, GlFloat))
    ...
    sf.buildProgram() match {
      case Success(p) => {
        ...
        sf
          ...
          .registerUniform("tex", new Sampler(0))
          .registerUniform("maxIterations", GlUInt)
          .buildProgram() match {
            case Success(p) => {
              renderProg = p
